...
(async () => {
  ...
  const handlers = {
    guest: {
      ...
      // *****modif du 26/10/25 Carillon+message vocal+flash ******

      // --- 1. Carillon synthétique (API AudioContext) ---
      try {
        ...
        playNote(880, 0.3); // La5
        playNote(1175, 0.3, 0.3); // Ré6
      } catch (e) {
        console.warn('Erreur lors du carillon audio:', e);
      }

      // --- 2. Message vocal "Portail activé" ---
      try {
        const utterance = new SpeechSynthesisUtterance('Portail activé');
        utterance.lang = 'fr-FR';
        utterance.rate = 1.0;
        speechSynthesis.speak(utterance);
      } catch (e) {
        console.warn('Erreur lors de la synthèse vocale:', e);
      }

      // --- 3. Flash visuel de l’écran ---
      try {
        ...
        console.warn('Erreur lors du flash visuel:', e);
      }

      // *****fin modif du 26/10/25 ******
          }
        } catch (error) {
